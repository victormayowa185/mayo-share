...
const ReceiveFromBrowser: React.FC<Props> = ({
  ...
  useEffect(() => {
    const unsubscribeUpload = window.electronAPI.onUploadUpdate((data) => {
    ...
    const unsubscribeSender = window.electronAPI.onSenderConnected(
    ...
    return () => {
      ...
      if (unsubscribeSender) unsubscribeSender();
    };
  }, []);
  ...
  const startReceiving = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || t("errorOccurred"));
      setStartingHotspot(false);
      setHotspotStatus("");
